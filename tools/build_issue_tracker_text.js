...
const build_issue_tracker_text = async (
  run_log,
  job_id,
  report_meta_data,
  reportable_data
  ...
  try {
    await addLogEvent(I, run_log, "build_issue_tracker_text", cal, note, null);
    ...
    for await (const rpp_data of reportable_data) {
      ...
      const col_4_data = {};
      if (!rpp_data.diagnoses) {
        col_4_data.diagnoses = "N/A";
      } else {
        col_4_data.diagnoses = rpp_data.diagnoses.slice(0, 50) + "...";
      }
    ...
    console.log(error);
    await addLogEvent(E, run_log, "build_issue_tracker_text", cat, note, error);
  }
};

module.exports = build_issue_tracker_text;
