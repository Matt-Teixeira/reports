const { build_missed_stack_mmb, build_full_email } = require("../tools");
...
const missed_stack_run_mmb = async (run_log, job_id, user_reports) => {
  let note = { job_id, user_report: user_reports };
  await addLogEvent(I, run_log, "missed_stack_run_mmb", cal, note, null);
  ...
  try {
    ...
    if (systems_list.length === 0) {
      ...
      await addLogEvent(W, run_log, "missed_stack_run_mmb", det, note, null);
      return;
    }
    await addLogEvent(I, run_log, "missed_stack_run_mmb", det, note, null);

    // 2) Build row text
    const email_text = await build_missed_stack_mmb(
      run_log,
      job_id,
      report_meta_data,
      systems_list
    ...
    console.log(error);
    await addLogEvent(E, run_log, "missed_stack_run_mmb", cat, note, error);
  }
};

module.exports = missed_stack_run_mmb;
