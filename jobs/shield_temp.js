...
const shield_temp = async (run_log, job_id, user_reports) => {
  let note = { job_id, user_report: user_reports };
  await addLogEvent(I, run_log, "shield_temp_report", cal, note, null);
  ...
  try {
    ...
    for (let rpp_data of user_reports.matched_systems_list) {
      ...
      rpp_data.rpp_value += " " + rpp_data.unit;
    ...
    if (sorted_data.length === 0) {
      ...
      await addLogEvent(W, run_log, "shield_temp_report", det, note, null);
      return;
    }
    await addLogEvent(I, run_log, "shield_temp_report", det, note, null);
    ...
    console.log(error);
    await addLogEvent(E, run_log, "shield_temp_report", cat, note, error);
  }
};

module.exports = shield_temp;
