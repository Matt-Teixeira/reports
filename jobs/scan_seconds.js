...
const scan_seconds = async (run_log, job_id, user_reports) => {
  let note = { job_id, user_report: user_reports };
  await addLogEvent(I, run_log, "scan_seconds_report", cal, note, null);
  ...
  try {
    ...
    if (sorted_data.length === 0) {
      ...
      await addLogEvent(W, run_log, "scan_seconds_report", det, note, null);
      return;
    }
    await addLogEvent(I, run_log, "scan_seconds_report", det, note, null);
    ...
    console.log(error);
    await addLogEvent(E, run_log, "scan_seconds_report", cat, note, error);
  }
};

module.exports = scan_seconds;
