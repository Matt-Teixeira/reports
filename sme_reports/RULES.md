...
| Scheduled (DB-config) runs | `npm start sme_report` with no file argument matches the current slot (shared `tools/schedule_dt.js`, the alert.reports convention) against **`alert.sme_reports`** template rows and fans out; `--slot` / `--config <id>` / `--dry-run` are the operator overrides. A `user_summary` row resolves its audience from `public.users` (active, `notify_email`, ≥1 magnet), **groups users by identical magnet scope, renders once per group**, and emails each user individually; a **send-time re-check** of the recipient's current `system_list_cache` (and status) skips anyone whose access no longer covers the document (`skipped_access`). New rows are **inert by construction** (`enabled` false + `dry_run` true); dry runs execute everything but SMTP and never archive history. Every delivery attempt writes one **`alert.sme_report_sends`** row (sent / error / skipped_access / dry_run) carrying the scope hash and document name; a failed group writes error rows for its whole intended audience so "did customer X get their report" is always answerable. Isolation: a failing row never sinks other rows, a failing scope-group never sinks other groups, and the process exits nonzero if ANY failed. Config `options` are a contract — unknown keys are rejected, never ignored |
