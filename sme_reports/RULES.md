...
| Report period (`lookback_days`) | A batch-level `lookback_days` (7 for the weekly customer product; 30 default) sets every report's window default; a per-report `window` — its own lookback or explicit dates — still wins. Non-default periods **tag every artifact name and email subject** (`-7d` on fleet/scoped/brief archive filenames, "7-day" in the summary subject): a weekly and a monthly produced the same day must never collide on disk or be indistinguishable in an inbox. Explicit-date windows carry no tag — the day count would be a coincidence, not a chosen period. Detection rules are period-agnostic; the left-censor never-urgent stance at 7 days is flagged for domain review in §6 terms and deliberately unchanged |
| Scoped (customer-facing) summary | A request-level `scope` (`{customer_id}` / `{site_ids}` / `{system_ids}`) resolves through `customers → sites → systems` (mag-processed only) — the same tables the identity query reads, so the join and the displayed names share one source. Resolution is **loud, never silent**: zero systems, unknown site ids, or non-mag system ids are fatal request errors, and the resolution (label + site/system counts) is logged and stated on the cover. The scoped document titles itself **"Magnet Health Summary — <label>"**, its rollup reads "% of these systems" (never "fleet"), and its NO REPORT PRODUCED reasons state the fact, not the pipeline ("no monitor data received…", vendor-variant tokens stripped). Exclusions stay stated on scoped documents — never-silent outranks tone. Files carry the scope slug (`Avante-<slug>-Magnet-Health-Summary-<date>`), collision-proof against the internal fleet document, which is byte-unchanged by all of this |
