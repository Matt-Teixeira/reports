...
const render_timeseries_chart = ({
  ...
  for (const tick of ticks) {
    ...
    parts.push(
      `<text x="${r1(sx(tick.t))}" y="146" font-size="8.5" fill="${COLORS.axis_text}" text-anchor="${tick.anchor}">${label}</text>`
    );
  }
  ...
  return `<svg viewBox="0 0 640 152" width="100%">${parts.join("")}</svg>`;
};
